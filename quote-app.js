(() => {
  ...
  function SetupScreen({ clients, onComplete }) {
    ...
    const handleSubmit = (e) => {
        ...
        onComplete({ pricingModel, clientDetails: clientData ? { name: `${clientData.first_name} ${clientData.last_name}`, company: customCompany, email: clientData.email } : { name: '', company: '' } });
    };
  ...
  function InvoiceGenerator({ setupData, suppliers }) {
    ...
    const handleBankPresetChange = (e) => {
        ...
        if (val !== 'custom') { const preset = PRESET_BANK_ACCOUNTS.find(p => p.id === val); if (preset) setBankDetails(preset.details); }
    };
    
    const handleFeeChange = (e) => { const { name, value, type, checked } = e.target; setFees(prev => ({ ...prev, [name]: type === 'checkbox' ? checked : parseFloat(value) || 0 })); };
    ...
    const calculations = useMemo(() => {
        ...
        const grandTotalBase = creditCardFeeInclusion === 'included' ? totalBeforeCCBase + ccFeeBase : totalBeforeCCBase;
        return { baseCurrency, clientCurrency, grandTotal: grandTotalBase * rate, ccFee: ccFeeBase * rate, otherFees: otherFeesBase * rate, ffiFee: ffiFeeBase * rate, rate };
    ...
    // DEDICATED PDF GENERATOR FUNCTION
      ...
      try {
        ...
        if (invoiceView === 'detailed') {
            tableRowsHTML = items.map(it => {
                ...
                return `<tr style="border-bottom:1px solid #f1f5f9"><td style="padding:16px 12px;font-size:14px;color:#0f172a;vertical-align:middle"><div style="display:flex;align-items:center;gap:12px">${catIcon?`<img src="${catIcon}" style="width:24px;height:24px;object-fit:contain" crossorigin="anonymous" />`:''}<div><strong style="display:block;margin-bottom:4px">${it.description||"—"}</strong>${supplierName?`<span style="color:#64748b;font-size:12px">Supplier: ${supplierName}</span>`:''}</div></div></td><td style="padding:16px 12px;text-align:right;font-size:14px;color:#475569;vertical-align:middle">${it.quantity}</td><td style="padding:16px 12px;text-align:right;font-size:14px;color:#475569;vertical-align:middle">${moneyClient(itemPriceClient)}</td><td style="padding:16px 12px;text-align:right;font-size:14px;font-weight:bold;color:#0f172a;vertical-align:middle">${moneyClient(itemTotalClient)}</td></tr>`;
            }).join('');
        } else {
            tableRowsHTML = `<tr style="border-bottom:1px solid #f1f5f9"><td colspan="3" style="padding:16px 12px;font-size:14px;color:#475569;white-space:pre-wrap">${summaryNotes}</td><td style="padding:16px 12px;text-align:right;font-size:14px;font-weight:bold;color:#0f172a">${moneyClient(calculations.grandTotal)}</td></tr>`;
        }
        ...
        if (fees.isUKPackage) feesHTML += `<tr><td colspan="3" style="padding:12px;text-align:right;font-size:14px;color:#475569;font-style:italic">FFI Fee (1%)</td><td style="padding:12px;text-align:right;font-size:14px;color:#0f172a">${moneyClient(calculations.ffiFee)}</td></tr>`;
        if (creditCardFeeInclusion === 'included' && fees.creditCardFee > 0) feesHTML += `<tr><td colspan="3" style="padding:12px;text-align:right;font-size:14px;color:#475569">Credit Card Fee (${fees.creditCardFee}%)</td><td style="padding:12px;text-align:right;font-size:14px;color:#0f172a">${moneyClient(calculations.ccFee)}</td></tr>`;

        let depositHTML = '';
        if (calculatedDepositAmount > 0) depositHTML = `<tr><td colspan="3" style="padding:16px 12px 8px;text-align:right;font-size:14px;color:#64748b">Deposit Due</td><td style="padding:16px 12px 8px;text-align:right;font-size:16px;font-weight:bold;color:#0f172a">${moneyClient(calculatedDepositAmount)}</td></tr><tr><td colspan="3" style="padding:8px 12px 16px;text-align:right;font-size:14px;color:#64748b">Balance Due</td><td style="padding:8px 12px 16px;text-align:right;font-size:16px;font-weight:bold;color:#0f172a">${moneyClient(calculations.grandTotal-calculatedDepositAmount)}</td></tr>`;

        const htmlTemplate = `<div style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;background:#fff;width:800px;padding:40px;box-sizing:border-box;color:#333"><div style="margin-bottom:50px">${companyLogoUrl?`<img src="${companyLogoUrl}" style="width:120px;height:auto;margin-bottom:24px" crossorigin="anonymous" />`:''}<h1 style="margin:0;font-size:38px;color:#0f172a;font-weight:bold">Payment Request</h1><p style="margin:8px 0 0 0;color:#64748b;font-size:16px"># ${quoteInfo.number||'Draft'}</p></div><table width="100%" style="margin-bottom:50px;border-collapse:collapse"><tr><td width="50%" style="vertical-align:top"><p style="margin:0 0 8px 0;font-size:12px;color:#64748b;font-weight:600;text-transform:uppercase;letter-spacing:.5px">Billed To</p><p style="margin:0;font-size:18px;font-weight:bold;color:#0f172a">${setupData.clientDetails.name}</p>${setupData.clientDetails.company?`<p style="margin:4px 0 0 0;font-size:14px;color:#475569">${setupData.clientDetails.company}</p>`:''}</td><td width="50%" style="text-align:right;vertical-align:bottom"><div style="font-size:14px;color:#475569;line-height:1.6"><p style="margin:0"><strong>Invoice Date:</strong> ${quoteInfo.date}</p>${quoteInfo.dueDate?`<p style="margin:0"><strong>Due Date:</strong> ${quoteInfo.dueDate}</p>`:''}</div></td></tr></table><table width="100%" style="border-collapse:collapse;margin-bottom:40px"><thead><tr style="background-color:#f8fafc"><th style="padding:12px;text-align:left;font-size:14px;color:#334155;border-bottom:2px solid #e2e8f0;font-weight:bold">Description</th><th style="padding:12px;text-align:right;font-size:14px;color:#334155;border-bottom:2px solid #e2e8f0;font-weight:bold">Qty</th><th style="padding:12px;text-align:right;font-size:14px;color:#334155;border-bottom:2px solid #e2e8f0;font-weight:bold">Price</th><th style="padding:12px;text-align:right;font-size:14px;color:#334155;border-bottom:2px solid #e2e8f0;font-weight:bold">Total</th></tr></thead><tbody>${tableRowsHTML}${feesHTML}</tbody><tfoot><tr><td colspan="3" style="padding:20px 12px 12px;text-align:right;font-size:18px;font-weight:bold;color:#0f172a;border-top:2px solid #cbd5e1">Grand Total</td><td style="padding:20px 12px 12px;text-align:right;font-size:18px;font-weight:bold;color:#0f172a;border-top:2px solid #cbd5e1">${moneyClient(calculations.grandTotal)}</td></tr>${depositHTML}</tfoot></table>${bankDetails?`<div style="margin-top:50px;padding-top:30px;border-top:1px solid #e2e8f0"><h3 style="font-size:16px;color:#334155;margin:0 0 12px 0">Payment Details</h3><p style="font-size:14px;color:#64748b;white-space:pre-wrap;margin:0;line-height:1.5">${bankDetails}</p></div>`:''}</div>`;
        ...
        const opt = { margin: 0, filename: `${quoteInfo.number||'invoice'}.pdf`, image: { type: "jpeg", quality: 1 }, html2canvas: { scale: 2, useCORS: true }, jsPDF: { unit: "in", format: "a4", orientation: "portrait" } };
        await window.html2pdf().set(opt).from(tempContainer).save();
        document.body.removeChild(tempContainer);
      } catch (err) { console.error("PDF Generation Error:", err); alert("There was an issue generating the PDF."); } finally { setIsExporting(false); }
    };
            ...
            <div className="tw-max-w-7xl tw-mx-auto tw-grid tw-grid-cols-1 lg:tw-grid-cols-5 tw-gap-8">
                <div className="lg:tw-col-span-2 tw-space-y-6">
                    ...
                    <ControlCard title="Display Options" defaultOpen={false}>
                        ...
                        {invoiceView === 'summary' && ( <div className="tw-mt-4"> <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-700 tw-mb-1">Summary Description</label> <textarea rows="3" className="tw-w-full tw-p-2 tw-border tw-border-slate-300 tw-rounded-md focus:tw-ring-2 focus:tw-ring-[#303350] focus:tw-border-[#303350]" value={summaryNotes} onChange={(e) => setSummaryNotes(e.target.value)} placeholder="Enter a custom description..." /> </div> )}
                    </ControlCard>
                    <ControlCard title="Currency & Exchange Rate" defaultOpen={false}>
                        ...
                        <div className="tw-mt-4"> <InputField icon={<ArrowRightLeft size={16}/>} label={`Exchange Rate (1 ${currencySettings.base} = ? ${currencySettings.client})`} type="number" value={currencySettings.rate} onChange={(e) => setCurrencySettings({...currencySettings, rate: parseFloat(e.target.value) || 0})} step="0.0001" disabled={currencySettings.base === currencySettings.client} /> <p className="tw-text-xs tw-text-slate-400 tw-mt-1">Connect this to Wized to auto-fetch rates.</p></div>
                    </ControlCard>
                            ...
                            {items.map((item) => (
                                <div key={item.id} className="tw-bg-slate-50/80 tw-p-4 tw-rounded-xl tw-border tw-border-slate-200">
                                        ...
                                        <div className="tw-flex-grow tw-mr-2">
                                            <div className="tw-grid tw-grid-cols-2 tw-gap-2">
                                                <div>
                                                    ...
                                                    <select className="tw-w-full tw-p-2 tw-border tw-border-slate-300 tw-rounded-md tw-bg-white tw-text-sm focus:tw-ring-[#303350]" value={item.category} onChange={(e) => handleUpdateItem(item.id, 'category', e.target.value)}>
                                                        {Object.keys(CATEGORIES).map(cat => <option key={cat} value={cat}>{cat}</option>)}
                                                    </select>
                                                </div>
                                                <div>
                                                    <label className="tw-block tw-text-xs tw-font-medium tw-text-slate-500 tw-mb-1">Supplier (optional)</label>
                                                    <SearchableSelect options={supplierOptions} value={item.supplierId} onChange={(val) => handleUpdateItem(item.id, 'supplierId', val)} placeholder="Search supplier..." emptyLabel="+ Add new supplier" />
                                                </div>
                                            </div>
                                        </div>
                                        <button onClick={() => handleDeleteItem(item.id)} className="tw-mt-5 tw-p-2 tw-text-slate-400 hover:tw-text-red-600 hover:tw-bg-red-50 tw-rounded-md tw-transition-colors"><Trash2 size={18} /></button>
                                    </div>
                                    ...
                                    <div className="tw-grid tw-grid-cols-3 tw-gap-3">
                                        ...
                                        {pricingModel === 'nett' ? <MiniInputField label="Nett Unit Cost" type="number" value={item.nettUnitCost} onChange={(e) => handleUpdateItem(item.id, 'nettUnitCost', e.target.value)} symbol={calculations.baseCurrency.symbol} /> : <MiniInputField label="Gross Unit Price" type="number" value={item.grossUnitCost || (Number(item.nettUnitCost)*(1+(Number(item.markup)/100)))} onChange={(e) => handleUpdateItem(item.id, 'grossUnitCost', e.target.value)} symbol={calculations.baseCurrency.symbol} />}
                                    ...
                                    <div className="tw-text-right tw-text-sm tw-mt-3 tw-font-medium tw-text-slate-600">Line total (Base): {moneyBase((item.quantity * item.nettUnitCost) * (1 + (item.markup/100)))}</div>
                                </div>
                            ))}
                            <button onClick={handleAddItem} className="tw-w-full tw-flex tw-items-center tw-justify-center tw-gap-2 tw-bg-slate-100 tw-text-slate-700 tw-border tw-border-slate-300 tw-py-3 tw-rounded-xl tw-font-semibold hover:tw-bg-slate-200 tw-transition-colors"><Plus size={18} /> Add Service</button>
                        </div>
                    </ControlCard>
                        ...
                        <div className="tw-space-y-4">
                            ...
                            <div className="tw-flex tw-items-center tw-justify-between">
                                ...
                                <div className="tw-flex tw-items-center tw-gap-3">
                                    ...
                                    <button onClick={() => setCreditCardFeeInclusion(prev => prev === 'included' ? 'separate' : 'included')} className={`tw-relative tw-inline-flex tw-h-6 tw-w-11 tw-flex-shrink-0 tw-cursor-pointer tw-rounded-full tw-border-2 tw-border-transparent tw-transition-colors tw-duration-200 tw-ease-in-out focus:tw-outline-none ${creditCardFeeInclusion === 'included' ? 'tw-bg-[#303350]' : 'tw-bg-gray-200'}`}><span className={`tw-pointer-events-none tw-inline-block tw-h-5 tw-w-5 tw-transform tw-rounded-full tw-bg-white tw-shadow tw-ring-0 tw-transition tw-duration-200 tw-ease-in-out ${creditCardFeeInclusion === 'included' ? 'tw-translate-x-5' : 'tw-translate-x-0'}`} /></button>
                    ...
                    <ControlCard title="Payments" defaultOpen={false}>
                        <div className="tw-flex tw-items-center tw-justify-between tw-mb-4">
                            ...
                            <div className="tw-flex tw-items-center tw-gap-3">
                                ...
                                <button onClick={() => setDepositType(prev => prev === 'amount' ? 'percentage' : 'amount')} className={`tw-relative tw-inline-flex tw-h-6 tw-w-11 tw-flex-shrink-0 tw-cursor-pointer tw-rounded-full tw-border-2 tw-border-transparent tw-transition-colors tw-duration-200 tw-ease-in-out focus:tw-outline-none ${depositType === 'amount' ? 'tw-bg-[#303350]' : 'tw-bg-gray-200'}`}><span className={`tw-pointer-events-none tw-inline-block tw-h-5 tw-w-5 tw-transform tw-rounded-full tw-bg-white tw-shadow tw-ring-0 tw-transition tw-duration-200 tw-ease-in-out ${depositType === 'amount' ? 'tw-translate-x-5' : 'tw-translate-x-0'}`} /></button>
                    ...
                    <button onClick={downloadPdf} disabled={isExporting} className="tw-w-full tw-bg-[#0b0e2c] tw-text-white tw-py-4 tw-rounded-xl tw-font-semibold hover:tw-opacity-90 tw-transition-opacity tw-text-lg tw-shadow-md disabled:tw-opacity-50">{isExporting ? 'Generating PDF...' : 'Export as PDF'}</button>
                </div>
                        ...
                        <div className="tw-bg-white tw-rounded-2xl tw-shadow-[0_8px_30px_rgb(0,0,0,0.04)] tw-border tw-border-slate-100 tw-p-8 sm:tw-p-12 tw-block">
                            <div className="tw-mb-8">
                                ...
                                <div className="tw-flex tw-justify-between tw-items-end">
                                    <div><h1 className="tw-text-4xl tw-font-bold tw-text-slate-900">Payment Request</h1><p className="tw-text-slate-500 tw-mt-2"># {quoteInfo.number}</p></div>
                                </div>
                            </div>
                            <div className="tw-flex tw-justify-between tw-items-end tw-border-b tw-border-slate-100 tw-pb-8 tw-mb-8"> 
                                <div><p className="tw-font-semibold tw-text-slate-400 tw-text-xs tw-uppercase tw-tracking-wider tw-mb-2">Billed To</p><p className="tw-font-bold tw-text-slate-900 tw-text-xl">{setupData.clientDetails.name}</p>{setupData.clientDetails.company && <p className="tw-text-slate-600 tw-mt-1">{setupData.clientDetails.company}</p>}</div>
                                <div className="tw-text-right tw-text-sm tw-text-slate-600 tw-space-y-1"><p><span className="tw-font-semibold">Invoice Date:</span> {quoteInfo.date}</p>{quoteInfo.dueDate && <p><span className="tw-font-semibold">Due Date:</span> {quoteInfo.dueDate}</p>}</div>
                            </div>
                            <div className="tw-mt-10">
                                <table className="tw-w-full tw-text-left">
                                    <thead><tr className="tw-bg-slate-50 tw-text-slate-700 tw-text-sm"><th className="tw-p-4 tw-font-semibold tw-rounded-l-xl">Description</th><th className="tw-p-4 tw-font-semibold tw-text-right">Qty</th><th className="tw-p-4 tw-font-semibold tw-text-right">Price</th><th className="tw-p-4 tw-font-semibold tw-text-right tw-rounded-r-xl">Total</th></tr></thead>
                                    <tbody>
                                        ...
                                        }) : <tr className="tw-border-b tw-border-slate-100"><td colSpan="3" className="tw-p-4 tw-text-slate-600 tw-whitespace-pre-wrap">{summaryNotes}</td><td className="tw-p-4 tw-text-right tw-font-semibold tw-text-slate-900 tw-align-middle">{moneyClient(calculations.grandTotal)}</td></tr>}
                                    ...
                                    <tfoot>
                                        <tr className="tw-border-t-2 tw-border-slate-200"><td className="tw-p-4 tw-font-bold tw-text-slate-900 tw-text-xl tw-text-right" colSpan="3">Grand Total</td><td className="tw-p-4 tw-text-right tw-font-bold tw-text-slate-900 tw-text-xl">{moneyClient(calculations.grandTotal)}</td></tr>
                                        {calculatedDepositAmount > 0 && <><tr className="tw-bg-slate-50"><td colSpan="3" className="tw-pt-4 tw-px-4 tw-text-right tw-text-sm tw-font-medium tw-text-slate-600">Deposit Due</td><td className="tw-pt-4 tw-px-4 tw-text-right tw-text-lg tw-font-bold tw-text-slate-900">{moneyClient(calculatedDepositAmount)}</td></tr><tr className="tw-bg-slate-50"><td colSpan="3" className="tw-pb-4 tw-px-4 tw-text-right tw-text-sm tw-font-medium tw-text-slate-600">Balance Due</td><td className="tw-pb-4 tw-px-4 tw-text-right tw-text-lg tw-font-bold tw-text-slate-900">{moneyClient(calculations.grandTotal - calculatedDepositAmount)}</td></tr></>}
                                    </tfoot>
                                </table>
                            </div>
                            {bankDetails && <div className="tw-mt-12 tw-pt-6 tw-border-t tw-border-slate-100"><h3 className="tw-font-semibold tw-text-slate-800 tw-mb-2">Payment Details</h3><p className="tw-text-slate-600 tw-text-sm tw-whitespace-pre-wrap tw-leading-relaxed">{bankDetails}</p></div>}
                        </div>
                    </div>
                </div>
            </div>
    ...
    useEffect(() => {
        (async () => {
            try {
                ...
                const [clientData, supplierData] = await Promise.all([execWizedRequestAndWait(WIZED_REQ.clients), execWizedRequestAndWait(WIZED_REQ.suppliers)]);
                setDbData({ clients: Array.isArray(clientData) ? clientData : [], suppliers: Array.isArray(supplierData) ? supplierData : [] });
            } catch (e) { console.error("Failed to load Wized data:", e); } finally { setLoading(false); }
        })();
    }, []);

    if (loading) return (
      <div className="tw-min-h-[70vh] tw-flex tw-items-center tw-justify-center"><lottie-player src="https://cdn.prod.website-files.com/656cafcf92ee678d635ab3dd/65afedc751a231c6ae634164_Animation%20-%201706028438496.json" background="transparent" speed="1" style={{ width: '450px', height: '450px' }} loop autoplay></lottie-player></div>
    );

    if (view === 'setup') return <SetupScreen clients={dbData.clients} onComplete={(data) => { setSetupData(data); setView('invoice'); }} />;
    return <InvoiceGenerator setupData={setupData} suppliers={dbData.suppliers} />;
  }

  const el = document.getElementById("quote-app");
  if (el) { const root = ReactDOM.createRoot(el); root.render(<App />); }
})();
