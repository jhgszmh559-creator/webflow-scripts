(() => {
  ...
  function InvoiceGenerator({ setupData, suppliers, onBack, savedInvoiceData, currentQuoteId }) {
      const [items, setItems] = useState(savedInvoiceData?.items || [
          { id: Date.now(), supplierId: "", category: "Hotel", description: '', nettUnitCost: 0, quantity: 1, markup: 20, isFullDeposit: false }
      ]);
      ...
      const [fees, setFees] = useState(savedInvoiceData?.fees || { creditCardFee: 0, otherFees: 0, isUKPackage: false });
      ...
      const [depositValue, setDepositValue] = useState(savedInvoiceData?.depositValue || 0);
      ...
      const handleAddItem = () => setItems([...items, { id: Date.now(), supplierId: "", category: "Hotel", description: '', nettUnitCost: 0, quantity: 1, markup: 20, isFullDeposit: false }]);
                          ...
                          <tr>
                              ...
                              <td width="50%" style="text-align: right; vertical-align: bottom;">
                                  <div style="font-size: 14px; color: #475569; line-height: 1.6;">
                                      ...
                                      ${depositDueDate ? `<p style="margin: 0;"><strong>Deposit Due:</strong> ${formatDate(depositDueDate)}</p>` : ''}
                                      ${balanceDueDate ? `<p style="margin: 0;"><strong>Balance Due:</strong> ${formatDate(balanceDueDate)}</p>` : ''}
                                  </div>
                              </td>
                          </tr>
                      </table>
...
const handleSaveQuote = async () => {
          ...
          try {
              ...
              const absoluteCommissionBase = items.reduce((sum, item) => sum + ((Number(item.nettUnitCost) * Number(item.quantity)) * (Number(item.markup) / 100)), 0);
                  ...
                  <div className="lg:tw-col-span-2 tw-space-y-6">
                      ...
                      <ControlCard title="Details" defaultOpen={true} isComplete={isDetailsComplete}>
                          <div className="tw-space-y-4">
                              ...
                              <div className="tw-grid tw-grid-cols-2 tw-gap-4">
                                  <InputField type="date" label="Travel Start Date" value={quoteInfo.travelStartDate || ''} onChange={(e) => setQuoteInfo({...quoteInfo, travelStartDate: e.target.value})} />
                                  <InputField type="date" label="Travel End Date" value={quoteInfo.travelEndDate || ''} onChange={(e) => setQuoteInfo({...quoteInfo, travelEndDate: e.target.value})} />
                              </div>
                              <div className="tw-grid tw-grid-cols-2 tw-gap-4">
                      ...
                      <ControlCard title="Currency & Exchange Rate" defaultOpen={false} isComplete={isCurrencyComplete}>
                          ...
                          <div className="tw-mt-4"> 
                              <InputField icon={<ArrowRightLeft size={16}/>} label={`Exchange Rate (1 ${currencySettings.base} = ? ${currencySettings.client})`} type="number" value={currencySettings.rate} onChange={(e) => setCurrencySettings({...currencySettings, rate: parseFloat(e.target.value) || 0})} step="0.0001" /> 
...
<div className="tw-grid tw-grid-cols-3 tw-gap-3 tw-items-end">
                                          ...
                                          {pricingModel === 'nett' ? (
                                              ...
                                              <MiniInputField label={item.category === 'Hotel' ? "Gross nightly price" : "Gross Unit Price"} type="number" value={item.grossUnitCost || (Number(item.nettUnitCost)*(1+(Number(item.markup)/100)))} onChange={(e) => handleUpdateItem(item.id, 'grossUnitCost', e.target.value)} symbol={calculations.baseCurrency.symbol} />
                                          )}
                                      ...
                                      <div className="tw-flex tw-items-center tw-justify-between tw-mt-4 tw-pt-4 tw-border-t tw-border-solid tw-border-slate-200/60">
                                          <div className="tw-flex tw-items-center tw-gap-2">
                                              ...
                                              />
                                              <label htmlFor={`deposit-${item.id}`} className="tw-text-sm tw-font-medium tw-text-slate-600 tw-m-0 tw-cursor-pointer">Require 100% upfront at deposit</label>
                                          </div>
...
<div className="tw-grid tw-grid-cols-2 tw-gap-1 tw-bg-slate-100 tw-p-1 tw-rounded-lg tw-w-full">
                                  {['summary', 'grouped', 'supplier', 'detailed'].map(v => (
                                      <button
                                          key={v}
                                          type="button"
                                          onClick={() => setInvoiceView(v)}
                                          className={`tw-w-full tw-text-center tw-text-sm tw-font-medium tw-py-2 tw-px-3 tw-rounded-md tw-transition-all tw-border-none tw-cursor-pointer ${invoiceView === v ? 'tw-bg-white tw-shadow-sm tw-text-[#303350]' : 'tw-bg-transparent tw-text-slate-500 hover:tw-text-slate-700'}`}
                                      >
                                          {v === 'grouped' ? 'By Category' : v === 'supplier' ? 'By Supplier' : v.charAt(0).toUpperCase() + v.slice(1)}
                                      </button>
                                  ))}
                              </div>
                              {invoiceView === 'summary' && ( 
                      ...
                      <ControlCard title="Additional Fees" defaultOpen={false} isComplete={isFeesComplete}>
                          <div className="tw-space-y-4">
                              <div className="tw-flex tw-items-center tw-justify-between tw-p-3 tw-bg-slate-50 tw-rounded-lg tw-border tw-border-solid tw-border-slate-200">
                                  <label htmlFor="isUKPackage" className="tw-text-sm tw-font-medium tw-text-slate-700 tw-m-0">UK Package trip? (adds 1% FFI)</label>
                      ...
                      <ControlCard title="Payments" defaultOpen={false} isComplete={isPaymentsComplete}>
                          <div className="tw-space-y-4">
                              ...
                              <div>
                                  <InputField label="Deposit Due Amount" type="number" value={depositValue} onChange={(e) => setDepositValue(parseFloat(e.target.value) || 0)} icon={depositType === 'amount' ? <span>{calculations.clientCurrency.symbol}</span> : null} symbol={depositType === 'percentage' ? '%' : null} />
                                  {depositType === 'percentage' && items.some(it => it.isFullDeposit) && (
                                      <p className="tw-text-xs tw-text-emerald-600 tw-font-medium tw-mt-1.5 tw-m-0">
                                          * Items marked "100% upfront" are automatically added to this percentage calculation.
                                      </p>
                                  )}
                              </div>
                              <div className="tw-grid tw-grid-cols-2 tw-gap-4">
                              ...
                              <InputField label="Payment Link (URL)" type="url" value={paymentLink} onChange={(e) => setPaymentLink(e.target.value)} placeholder="https://buy.stripe.com/..." />
                          </div>
                      </ControlCard>
                          ...
                          <div className="tw-bg-white tw-rounded-2xl tw-shadow-[0_8px_30px_rgb(0,0,0,0.04)] tw-border tw-border-solid tw-border-slate-100 tw-p-8 sm:tw-p-12 tw-block">
                              ...
                              <div className="tw-flex tw-justify-between tw-items-end tw-border-b tw-border-solid tw-border-slate-100 tw-pb-8 tw-mb-8"> 
                                  ...
                                  <div className="tw-text-right tw-text-sm tw-text-slate-600 tw-space-y-1">
                                      ...
                                      {depositDueDate && <p className="tw-m-0"><span className="tw-font-semibold">Deposit Due:</span> {formatDate(depositDueDate)}</p>}
                                      {balanceDueDate && <p className="tw-m-0"><span className="tw-font-semibold">Balance Due:</span> {formatDate(balanceDueDate)}</p>}
                                  </div>
                              </div>
                              <div className="tw-mt-10">
                                  <table className="tw-w-full tw-text-left tw-border-collapse">
                                      <thead><tr className="tw-bg-slate-50 tw-text-slate-700 tw-text-sm"><th className="tw-p-4 tw-font-semibold tw-rounded-l-xl tw-border-b tw-border-solid tw-border-slate-100">Description</th><th className="tw-p-4 tw-font-semibold tw-text-right tw-border-b tw-border-solid tw-border-slate-100">Qty</th><th className="tw-p-4 tw-font-semibold tw-text-right tw-border-b tw-border-solid tw-border-slate-100">Price</th><th className="tw-p-4 tw-font-semibold tw-text-right tw-rounded-r-xl tw-border-b tw-border-solid tw-border-slate-100">Total</th></tr></thead>
                                      <tbody>
...
{invoiceView === 'grouped' && Object.entries(items.reduce((acc, it) => {
                                              ...
                                              <tr key={cat} className="tw-border-b tw-border-solid tw-border-slate-100 last:tw-border-0">
                                                  ...
                                                  <td className="tw-p-4 tw-text-right tw-font-semibold tw-text-slate-900 tw-align-middle">{moneyClient(total)}</td>
                                              </tr>
                                          ))}
                                          ...
                                          {invoiceView === 'summary' && <tr className="tw-border-b tw-border-solid tw-border-slate-100"><td colSpan="3" className="tw-p-4 tw-text-slate-600 tw-whitespace-pre-wrap">{summaryNotes}</td><td className="tw-p-4 tw-text-right tw-font-semibold tw-text-slate-900 tw-align-middle">{moneyClient(calculations.grandTotal)}</td></tr>}
                                      ...
                                      <tfoot>
                                          <tr className="tw-border-t-2 tw-border-solid tw-border-slate-200"><td className="tw-p-4 tw-font-bold tw-text-slate-900 tw-text-xl tw-text-right" colSpan="3">Grand Total</td><td className="tw-p-4 tw-text-right tw-font-bold tw-text-slate-900 tw-text-xl">{moneyClient(calculations.grandTotal)}</td></tr>
