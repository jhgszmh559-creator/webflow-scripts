(() => {
  ...
  const FX_MARGIN_TABLE = {
      ...
      GBP: { USD: 0.025, EUR: 0.022, ZAR: 0.38,  CAD: 0.033 },
      EUR: { GBP: 0.015, USD: 0.02,  ZAR: 0.35,  CAD: 0.0275 },
      ZAR: { USD: 0.005, GBP: 0.005, EUR: 0.005, CAD: 0.005 },
      CAD: { USD: 0.15,  GBP: 0.015, EUR: 0.015, ZAR: 0.205 },
      JPY: { USD: 0.00015, GBP: 0.00009368, EUR: 0.00012, CAD: 0.00015, ZAR: 0.002 },
      AUD: { USD: 0.015, GBP: 0.012, EUR: 0.012 },
      CHF: { USD: 0.025, GBP: 0.018, EUR: 0.02 },
  };
                  ...
                  <div className="lg:tw-col-span-2 tw-space-y-6">
                      ...
                      <ControlCard title="Currency & Exchange Rate" defaultOpen={false} isComplete={isCurrencyComplete}>
                          ...
                          <div className="tw-mt-4">
                              <InputField icon={<ArrowRightLeft size={16}/>} label={`Exchange Rate (1 ${currencySettings.base} = ? ${currencySettings.client})`} type="number" value={currencySettings.rate} onChange={(e) => setCurrencySettings({...currencySettings, rate: e.target.value})} step="0.0001" />
                              <div className="tw-mt-1.5 tw-flex tw-items-center tw-justify-between tw-gap-2">
                                  <div className="tw-text-xs tw-text-slate-500 tw-leading-snug">
                                      {fxStatus.loading && <span>Fetching live rate…</span>}
                                      {!fxStatus.loading && fxStatus.error && (
                                          <span className="tw-text-red-500">Live rate unavailable: {fxStatus.error}. Edit manually.</span>
                                      )}
                                      {!fxStatus.loading && !fxStatus.error && fxStatus.spotRate != null && currencySettings.base !== currencySettings.client && (
                                          <span>
                                              Live spot {fxStatus.spotRate.toFixed(4)} + CT margin {fxStatus.margin.toFixed(4)} = <strong>{(fxStatus.spotRate + fxStatus.margin).toFixed(4)}</strong>.
                                              {' '}You can override manually.
                                          </span>
                                      )}
                                      {!fxStatus.loading && !fxStatus.error && currencySettings.base === currencySettings.client && (
                                          <span>Same currency — rate fixed at 1.</span>
                                      )}
                                  </div>
                                  <button
                                      type="button"
                                      onClick={() => refreshLiveRate(currencySettings.base, currencySettings.client)}
                                      disabled={fxStatus.loading}
                                      className="tw-text-xs tw-font-medium tw-text-[#303350] hover:tw-underline tw-bg-transparent tw-border-none tw-cursor-pointer tw-shrink-0 disabled:tw-opacity-50"
                                  >
                      ...
                      <ControlCard title="Additional Fees" defaultOpen={false} isComplete={isFeesComplete}>
                          <div className="tw-space-y-4">
                              ...
                              <div className="tw-flex tw-items-center tw-justify-between">
                                  <label className="tw-text-sm tw-font-medium tw-text-slate-700 tw-m-0">Fees</label>
                              ...
                              <div className="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4">
                                  <div>
                                      <div className="tw-flex tw-items-center tw-mb-1.5">
                                          <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-700 tw-m-0">CC Fee</label>
                                          <div className="tw-relative tw-group tw-inline-flex tw-items-center tw-cursor-pointer tw-ml-1.5">
                                              <span className="tw-text-slate-400 group-hover:tw-text-[#303350] tw-transition-colors">
                                                  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>
                                              </span>
                                      ...
                                      <select
                                          ...
                                          className="tw-w-full tw-p-2 tw-border tw-border-solid tw-border-slate-300 tw-rounded-md focus:tw-ring-2 focus:tw-ring-[#303350] focus:tw-border-[#303350] tw-bg-white"
                                      >
                                      ...
                                      {creditCardFeePreset === 'custom' && (
                                          <div className="tw-mt-2 tw-relative">
                                              <input
                                                  ...
                                                  step="0.1"
                                                  placeholder="Enter %"
                                                  className="tw-w-full tw-p-2 tw-pr-7 tw-border tw-border-solid tw-border-slate-300 tw-rounded-md focus:tw-ring-2 focus:tw-ring-[#303350] focus:tw-border-[#303350] tw-text-right"
                                              />
                                              <span className="tw-absolute tw-right-3 tw-top-1/2 tw--translate-y-1/2 tw-text-slate-400">%</span>
                                          </div>
                                      )}
                                  </div>
