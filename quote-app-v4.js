(() => {
  ...
  const CURRENCIES = [{ code: 'USD', symbol: '$', name: 'US Dollar' }, { code: 'EUR', symbol: '€', name: 'Euro' }, { code: 'GBP', symbol: '£', name: 'British Pound' }, { code: 'JPY', symbol: '¥', name: 'Japanese Yen' }, { code: 'AUD', symbol: 'A$', name: 'Australian Dollar' }, { code: 'CAD', symbol: 'C$', name: 'Canadian Dollar' }, { code: 'CHF', symbol: 'CHF', name: 'Swiss Franc' }];
  ...
  const ControlCard = ({ title, children, defaultOpen = true }) => {
      ...
      return (
          <div className="tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-slate-200 tw-overflow-hidden tw-transition-colors tw-duration-300"> 
              <div 
                  className="tw-px-6 tw-py-4 tw-flex tw-justify-between tw-items-center tw-cursor-pointer tw-bg-white tw-transition-colors"
                  onClick={() => setIsOpen(!isOpen)}
              >
                  <h2 className="tw-text-[18px] tw-font-bold tw-text-slate-800">{title}</h2>
                  <div className="tw-text-slate-400">
                      {isOpen ? <ArrowUp size={20} /> : <ArrowDown size={20} />}
                  </div>
              </div>
              {isOpen && (
                  <div className="tw-px-6 tw-pb-6 tw-pt-2 tw-border-t tw-border-slate-100">
                      {children}
                  </div>
              )}
          </div> 
  ...
  const InputField = ({ label, symbol, icon, ...props }) => (
      <div>
          <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-600 tw-mb-1.5">{label}</label>
  ...
  const MiniInputField = ({ label, symbol, ...props }) => ( 
      <div> 
          ...
          <div className="tw-relative"> 
              ...
              <input {...props} className={`tw-w-full tw-p-2 tw-border tw-border-slate-300 tw-rounded-md focus:tw-ring-2 focus:tw-ring-[#303350] focus:tw-border-[#303350] ${symbol && symbol !== '%' ? 'tw-pl-7' : ''} ${symbol === '%' ? 'tw-pr-7 tw-text-right' : ''}`} /> 
          </div> 
      </div> 
  );
              ...
              {isOpen && (
                  <div className="tw-absolute tw-z-50 tw-w-full tw-mt-1 tw-bg-white tw-border tw-border-slate-200 tw-rounded-md tw-shadow-lg tw-max-h-60 tw-overflow-auto">
                      {filteredOptions.length === 0 ? (
                          <div className="tw-p-3 tw-text-sm tw-text-[#303350] tw-font-medium tw-cursor-pointer hover:tw-bg-slate-50" onClick={() => { alert("Trigger Wized flow to create new item here."); setIsOpen(false); }}>
                              {emptyLabel}
                          </div>
                      ) : (
                          filteredOptions.map(opt => (
                              <div key={opt.value} className="tw-p-3 tw-text-sm hover:tw-bg-slate-50 tw-cursor-pointer tw-text-slate-700 tw-border-b tw-border-slate-50 last:tw-border-0" onClick={() => { onChange(opt.value); setIsOpen(false); setQuery(''); }}>
                                  {opt.label}
                              </div>
                          ))
                      )}
                      ...
                      <div className="tw-bg-white tw-p-8 sm:tw-p-10 tw-rounded-2xl tw-shadow-[0_8px_30px_rgb(0,0,0,0.04)] tw-border tw-border-slate-100 tw-space-y-10">
                          ...
                          <div>
                              ...
                              <div className="tw-grid tw-grid-cols-2 tw-gap-6">
                                  <div 
                                      ...
                                      className="tw-p-5 tw-rounded-xl tw-cursor-pointer tw-transition-all tw-bg-slate-50 hover:tw-bg-slate-100"
                                      style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: pricingModel === 'nett' ? '#303350' : '#e2e8f0' }}
                                  >
                                      ...
                                      <h3 className="tw-text-lg tw-font-bold tw-mb-2 tw-text-slate-800">Nett Pricing</h3>
                                      <p className="tw-text-sm tw-text-slate-500">Enter the cost to you (nett) and add your markup.</p>
                                  </div>
                                  <div 
                                      onClick={() => setPricingModel('gross')} 
                                      className="tw-p-5 tw-rounded-xl tw-cursor-pointer tw-transition-all tw-bg-slate-50 hover:tw-bg-slate-100"
                                      style={{ borderWidth: '1px', borderStyle: 'solid', borderColor: pricingModel === 'gross' ? '#303350' : '#e2e8f0' }}
                                  >
                                      <img src="https://cdn.prod.website-files.com/656cafcf92ee678d635ab3dd/6611ac28353fd48ae22ce9e5_arrow%20right.png" className="tw-w-6 tw-h-6 tw-mb-3" alt="icon" />
                                      <h3 className="tw-text-lg tw-font-bold tw-mb-2 tw-text-slate-800">Gross Pricing</h3>
                          ...
                          <div>
                              ...
                              <div className="tw-space-y-5">
                                  <div>
                                      <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-600 tw-mb-1.5">Client Name</label>
                                      <SearchableSelect 
  ...
  function InvoiceGenerator({ setupData, suppliers, onBack }) {
      ...
      const downloadPdf = async () => {
        ...
        try {
          ...
          const htmlTemplate = `
              <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background: #ffffff; width: 800px; padding: 40px; box-sizing: border-box; color: #333;">
          ...
          // Put the container in the DOM so html2canvas can 'see' it accurately, but hide it behind the app with z-index
          const tempContainer = document.createElement('div');
          tempContainer.innerHTML = htmlTemplate;
          tempContainer.style.position = 'absolute';
          tempContainer.style.top = '0';
          tempContainer.style.left = '0';
          tempContainer.style.zIndex = '-10'; // Behind visible page
          document.body.appendChild(tempContainer); 
          
          // Wait briefly for cross-origin images to load in the DOM before snapping the picture
          await sleep(500);
          ...
          <div className="tw-font-sans tw-bg-transparent tw-min-h-screen tw-p-2 lg:tw-p-4">
              <div className="tw-max-w-7xl tw-mx-auto tw-grid tw-grid-cols-1 lg:tw-grid-cols-5 tw-gap-8">
                  ...
                  <div className="lg:tw-col-span-2 tw-space-y-6">
                      ...
                      <ControlCard title="Details" defaultOpen={true}>
                      ...
                      <ControlCard title="Display Options" defaultOpen={false}>
                          ...
                          {invoiceView === 'summary' && ( 
                              <div className="tw-mt-4"> 
                                  <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-700 tw-mb-1.5">Summary Description</label> 
                      ...
                      <ControlCard title={`Travel Services (${calculations.baseCurrency.code} Pricing)`}>
                          <div className="tw-space-y-4">
                              {items.map((item) => (
                                  <div key={item.id} className="tw-bg-slate-50/80 tw-p-5 tw-rounded-xl tw-border tw-border-slate-200">
                                      
                                      <div className="tw-space-y-4 tw-mb-4">
                                          <div className="tw-flex tw-justify-between tw-items-end tw-gap-4">
                                              <div className="tw-flex-grow">
                                                  <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-600 tw-mb-1.5">Category</label>
                                                  <select 
                                              ...
                                              <button 
                                                  ...
                                                  className="tw-text-[#303350] hover:tw-text-red-500 tw-p-2 tw-transition-colors tw-flex-shrink-0 tw-mb-0.5" 
                                          ...
                                          <div>
                                              <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-600 tw-mb-1.5">Supplier (optional)</label>
                                              <SearchableSelect 
                                      ...
                                      <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-600 tw-mb-1.5">Description</label>
                                      ...
                                      <div className="tw-text-right tw-text-sm tw-mt-4 tw-font-semibold tw-text-[#303350]">
                      ...
                      <ControlCard title="Bank Details" defaultOpen={false}>
                          <div className="tw-space-y-4">
                              <div>
                                  <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-600 tw-mb-1.5">Account Preset</label>
