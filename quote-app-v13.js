(() => {
  ...
  async function execWizedRequestAndWait(reqName, { timeoutMs = 20000 } = {}) {
      ...
      await sleep(150); // Small buffer to ensure Wized registers the active request
      const start = Date.now();
      while (Date.now() - start < timeoutMs) {
  ...
  // PRESET AGENCY EXCHANGE RATES (Relative to 1 USD)
  // You can update these numbers whenever you want to change your standard agency rates!
  ...
  function InvoiceGenerator({ setupData, suppliers, onBack }) {
      ...
      const [currencySettings, setCurrencySettings] = useState({ base: 'USD', client: 'EUR', rate: 0.95 }); // Default 0.95 based on preset
      ...
      // Handle Preset Currency Changes
      const handleCurrencyChange = (field, newCurrency) => {
          setCurrencySettings(prev => {
              ...
              if (newSettings.base === newSettings.client) {
                  ...
                  // Calculate cross rate: (1 / Base_to_USD) * Client_to_USD
                  ...
                  <div className="lg:tw-col-span-2 tw-space-y-6">
                      ...
                      <ControlCard title="Display Options" defaultOpen={false} isComplete={isDisplayComplete}>
                          <div className="tw-flex tw-items-center tw-justify-between">
                              <label className="tw-text-sm tw-font-medium tw-text-slate-700 tw-m-0">Invoice View</label>
                      ...
                      <button type="button" onClick={handlePrintPdf} className="tw-w-full tw-bg-[#0b0e2c] tw-text-white tw-py-4 tw-rounded-xl tw-font-semibold hover:tw-opacity-90 tw-transition-opacity tw-text-lg tw-shadow-md tw-border-none">
                          Export as PDF
                      </button>
                  </div>
                      ...
                      <div className="tw-sticky tw-top-8">
                          <div className="tw-bg-white tw-rounded-2xl tw-shadow-[0_8px_30px_rgb(0,0,0,0.04)] tw-border tw-border-solid tw-border-slate-100 tw-p-8 sm:tw-p-12 tw-block">
                              ...
                              <div className="tw-flex tw-justify-between tw-items-end tw-border-b tw-border-solid tw-border-slate-100 tw-pb-8 tw-mb-8"> 
                                  <div><p className="tw-font-semibold tw-text-slate-400 tw-text-xs tw-uppercase tw-tracking-wider tw-mb-2 tw-m-0">Billed To</p><p className="tw-font-bold tw-text-slate-900 tw-text-xl tw-m-0">{setupData.clientDetails.name}</p>{setupData.clientDetails.company && <p className="tw-text-slate-600 tw-mt-1 tw-m-0">{setupData.clientDetails.company}</p>}</div>
                                  <div className="tw-text-right tw-text-sm tw-text-slate-600 tw-space-y-1"><p className="tw-m-0"><span className="tw-font-semibold">Invoice Date:</span> {quoteInfo.date}</p>{quoteInfo.dueDate && <p className="tw-m-0"><span className="tw-font-semibold">Due Date:</span> {quoteInfo.dueDate}</p>}</div>
                              </div>
                              <div className="tw-mt-10">
                                  <table className="tw-w-full tw-text-left tw-border-collapse">
                                      ...
                                      <tbody>
                                          ...
                                          }) : <tr className="tw-border-b tw-border-solid tw-border-slate-100"><td colSpan="3" className="tw-p-4 tw-text-slate-600 tw-whitespace-pre-wrap">{summaryNotes}</td><td className="tw-p-4 tw-text-right tw-font-semibold tw-text-slate-900 tw-align-middle">{moneyClient(calculations.grandTotal)}</td></tr>}
      ...
      useEffect(() => {
          ...
          (async () => {
              try {
                  ...
                  // 🔄 BACKGROUND AUTO-SYNC: Watch Wized data for changes
          ...
          <>
              ...
              {setupData && (
                  <div style={{ display: view === 'invoice' ? 'block' : 'none' }}>
                      <ErrorBoundary>
                          <InvoiceGenerator setupData={setupData} suppliers={dbData.suppliers} onBack={() => setView('setup')} />
                      </ErrorBoundary>
                  </div>
              )}
          </>
