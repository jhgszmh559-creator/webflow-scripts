...
// =========================
  ...
  function InvoiceGenerator({ setupData, suppliers, onBack, savedInvoiceData, currentQuoteId }) {
      // Use saved data if it exists, otherwise use defaults
      ...
      const [quoteInfo, setQuoteInfo] = useState(savedInvoiceData?.quoteInfo || { number: `Q-${Date.now().toString().slice(-6)}`, date: new Date().toISOString().split('T')[0], dueDate: '' });
...
const handleSaveQuote = async () => {
          ...
          try {
              const payload = {
                  ...
                  currency: currencySettings.client, // <-- Added this line!
                  status: "Draft",
                  app_data: {
                      setupData,
                      quoteInfo,
              ...
              if (window.Wized && window.Wized.data) {
                  ...
                  // Traffic Cop: Decide which Wized request to fire
                  if (currentQuoteId) {
                  ...
                  // Success! Redirect the user to the All Quotes page
              ...
              setIsSaving(false); // Only turn off loading if it failed (if successful, they are redirecting away anyway)
          }
      };
      return (
