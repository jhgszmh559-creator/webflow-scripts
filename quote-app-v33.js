          ...
          <div className="tw-flex tw-items-center tw-mb-1.5 tw-m-0">
              ...
              {tooltipLink && (
                  <div className="tw-relative tw-group tw-inline-flex tw-items-center tw-cursor-pointer tw-ml-1.5">
                      ...
                      {/* Tooltip container with invisible padding bridge so mouse hover doesn't drop */}
...
<div className="tw-mt-4"> 
                              <InputField icon={<ArrowRightLeft size={16}/>} label={`Exchange Rate (1 ${currencySettings.base} = ? ${currencySettings.client})`} tooltipLink="https://drive.google.com/drive/u/3/folders/0ACXz2hC43zP7Uk9PVA" type="number" value={currencySettings.rate} onChange={(e) => setCurrencySettings({...currencySettings, rate: parseFloat(e.target.value) || 0})} step="0.0001" /> 
                              ...
                              <div className="tw-mt-10">
                                  <table className="tw-w-full tw-text-left tw-border-collapse">
                                      ...
                                      <tbody>
                                          ...
                                          {fees.isUKPackage && <tr><td colSpan="3" className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-600 tw-italic">FFI Fee (1.25%)</td><td className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-900">{moneyClient(calculations.ffiFee)}</td></tr>}
                                          {creditCardFeeInclusion === 'separate' && fees.creditCardFee > 0 && <tr><td colSpan="3" className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-600">Credit Card Fee ({fees.creditCardFee}%)</td><td className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-900">{moneyClient(calculations.ccFee)}</td></tr>}
                                      </tbody>
                                      <tfoot>
