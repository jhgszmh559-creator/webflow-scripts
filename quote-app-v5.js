(() => {
  ...
  const ControlCard = ({ title, children, defaultOpen = true, isComplete = false }) => {
      ...
      return (
          <div className={`tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-solid tw-overflow-hidden tw-transition-colors tw-duration-300 ${isComplete ? 'tw-border-emerald-500' : 'tw-border-slate-200'}`}> 
              ...
              >
                  {/* tw-m-0 and tw-leading-none kill Webflow's default h2 margins */}
  ...
  const InputField = ({ label, symbol, icon, ...props }) => (
      <div>
          {/* Label styling optimized to match your screenshot */}
                      ...
                      <div className="tw-bg-white tw-p-8 sm:tw-p-10 tw-rounded-2xl tw-shadow-[0_8px_30px_rgb(0,0,0,0.04)] tw-border tw-border-solid tw-border-slate-100 tw-space-y-10">
                          ...
                          <div>
                              ...
                              <div className="tw-grid tw-grid-cols-2 tw-gap-6">
                                  {/* HARDCODED INLINE BORDER STYLES TO BEAT WEBFLOW */}
                                  <div 
  ...
  function InvoiceGenerator({ setupData, suppliers, onBack }) {
      ...
      // NATIVE BROWSER PRINT APPROACH (100% Reliable, Vector, Selectable Text)
      const handlePrintPdf = () => {
          ...
          // Wait half a second for images to render in the new tab, then print
          setTimeout(() => {
              printWin.focus();
              printWin.print();
          }, 500);
                              ...
                              {items.map((item) => (
                                  <div key={item.id} className="tw-bg-slate-50/80 tw-p-5 tw-rounded-xl tw-border tw-border-solid tw-border-slate-200">
                                      ...
                                      <div className="tw-flex tw-justify-between tw-items-center tw-mb-3">
                                          <label className="tw-block tw-text-sm tw-font-medium tw-text-slate-700 tw-m-0">Category</label>
                                          <button 
                                              type="button" 
                                              onClick={() => handleDeleteItem(item.id)} 
                                              className="tw-flex tw-items-center tw-justify-center tw-text-[#303350] hover:tw-text-red-500 tw-p-1 tw-transition-colors" 
