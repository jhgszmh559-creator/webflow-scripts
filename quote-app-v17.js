...
const [isSaving, setIsSaving] = useState(false);
      ...
      const handleSaveQuote = async () => {
          ...
          try {
              // 1. Package the data exactly as Xano/Wized expects it
              const payload = {
              ...
              // 2. Inject it into the Wized variable 'quote_payload'
              if (window.Wized && window.Wized.data) {
                  if (!window.Wized.data.v) window.Wized.data.v = {};
                  window.Wized.data.v.quote_payload = payload;

                  // 3. Trigger the Wized request
                  await Wized.requests.execute('save_new_quote'); // Make sure this matches your Wized request name!
              ...
              alert("There was an issue saving the quote. Check the console for details.");
          } finally {
              setIsSaving(false);
          }
      };
