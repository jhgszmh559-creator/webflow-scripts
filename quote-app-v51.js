...
const handleSaveQuote = async (options = {}) => {
          ...
          try {
              ...
              const payload = {
                  ...
                  status: "Draft",
                  // UK Package trip (the FFI checkbox). Sent as a top-level boolean so XANO can
                  // map it straight to the `package` column — previously it only existed nested
                  // at app_data.fees.isUKPackage, so the column was never set.
                  package: !!fees.isUKPackage,
...
// =========================
  ...
  function App() {
      ...
      // Set when the URL contains ?action=duplicate — prompts the user to duplicate the loaded quote.
      const [duplicatePrompt, setDuplicatePrompt] = useState(false);
      // Bumped to force the editor to re-mount fresh after a duplicate is confirmed.
      ...
      useEffect(() => {
          ...
          (async () => {
              try {
                  ...
                  // 1. Check URL for an existing quote ID (and whether we arrived in "duplicate" mode)
                  const urlParams = new URLSearchParams(window.location.search);
                  const qId = urlParams.get('quote_id');
                  const isDuplicateAction = urlParams.get('action') === 'duplicate';
                  ...
                  if (isMounted) {
                      ...
                      if (qId && quoteRecord && quoteRecord.app_data) {
                          ...
                          // If ?action=duplicate, ask the user whether they want to duplicate this quote.
                          if (isDuplicateAction) setDuplicatePrompt(true);
                      }
      ...
      // Turn the loaded quote into a brand-new draft the user can edit, then save as its own quote.
      // Mirrors the old "Copy quote": fresh number, no quote_id (so Save creates a new record).
      const handleConfirmDuplicate = () => {
          const newNumber = `Q-${Date.now().toString().slice(-6)}`;
          setSavedInvoiceData(prev => prev ? { ...prev, quoteInfo: { ...(prev.quoteInfo || {}), number: newNumber } } : prev);
          setCurrentQuoteId(null);            // new record on save
          setEditorKey(k => k + 1);           // remount the editor with the duplicated data
          setDuplicatePrompt(false);
          // Clean the URL so a refresh doesn't re-trigger the duplicate prompt.
      ...
      return (
          <>
              ...
              {duplicatePrompt && (
                  <div className="tw-fixed tw-inset-0 tw-z-[9999] tw-flex tw-items-center tw-justify-center tw-p-4" style={{ backgroundColor: 'rgba(15, 23, 42, 0.55)' }}>
                      <div className="tw-bg-white tw-rounded-2xl tw-shadow-2xl tw-max-w-md tw-w-full tw-p-8 tw-text-center">
                          <div className="tw-mx-auto tw-mb-4 tw-flex tw-h-12 tw-w-12 tw-items-center tw-justify-center tw-rounded-full tw-bg-slate-100 tw-text-[#0b0e2c]">
                              <Copy size={24} />
                          </div>
                          <h2 className="tw-text-xl tw-font-bold tw-text-slate-900 tw-m-0 tw-mb-2">Duplicate this quote?</h2>
                          <p className="tw-text-sm tw-text-slate-500 tw-m-0 tw-mb-6">
                              We'll create an editable copy with a new quote number. Your changes are saved as a brand-new quote — the original stays untouched.
                          </p>
                          <div className="tw-flex tw-flex-col sm:tw-flex-row tw-gap-3">
                              <button
                                  type="button"
                                  onClick={() => { window.location.href = '/booking-tracker/all-quotes'; }}
                                  className="tw-flex-1 tw-bg-white tw-text-slate-600 tw-py-3 tw-rounded-xl tw-font-semibold hover:tw-bg-slate-50 tw-transition-colors tw-cursor-pointer"
                              ...
                              <button
                                  ...
                                  className="tw-flex-1 tw-bg-[#0b0e2c] tw-text-white tw-py-3 tw-rounded-xl tw-font-semibold hover:tw-opacity-90 tw-transition-opacity tw-border-none tw-cursor-pointer tw-shadow-md"
                              >
                                  Duplicate quote
                              </button>
                          </div>
