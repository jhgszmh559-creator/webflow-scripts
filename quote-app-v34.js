(() => {
  ...
  function InvoiceGenerator({ setupData, suppliers, onBack, savedInvoiceData, currentQuoteId }) {
      const [items, setItems] = useState(savedInvoiceData?.items || [
          { id: Date.now(), supplierId: "", category: "Hotel", description: '', nettUnitCost: '', quantity: 1, markup: 20, isFullDeposit: false }
      ]);
      ...
      const isTravelComplete = items.length > 0 && items.every(i => i.description && i.quantity > 0 && Number(i.nettUnitCost || 0) >= 0);
      const isFeesComplete = true; 
      const isPaymentsComplete = depositValue !== null && depositValue !== '';
      const isBankComplete = !!bankDetails;

      const handleAddItem = () => setItems([...items, { id: Date.now(), supplierId: "", category: "Hotel", description: '', nettUnitCost: '', quantity: 1, markup: 20, isFullDeposit: false }]);
      ...
      const handlePrintPdf = () => {
          ...
          const ccMultiplier = creditCardFeeInclusion === 'included' ? (1 + ((Number(fees.creditCardFee) || 0) / 100)) : 1;
          const showFFILine = fees.isUKPackage && invoiceView !== 'summary' && invoiceView !== 'grouped';
          ...
          if (Number(fees.otherFees || 0) > 0) feesHTML += `<tr><td colspan="3" style="padding: 12px; text-align: right; font-size: 14px; color: #475569;">Other Fees</td><td style="padding: 12px; text-align: right; font-size: 14px; color: #0f172a;">${moneyClient(displayOtherFees)}</td></tr>`;
          if (showFFILine) feesHTML += `<tr><td colspan="3" style="padding: 12px; text-align: right; font-size: 14px; color: #475569; font-style: italic;">FFI Fee (1.12%)</td><td style="padding: 12px; text-align: right; font-size: 14px; color: #0f172a;">${moneyClient(calculations.ffiFee)}</td></tr>`;
          if (creditCardFeeInclusion === 'separate' && Number(fees.creditCardFee || 0) > 0) feesHTML += `<tr><td colspan="3" style="padding: 12px; text-align: right; font-size: 14px; color: #475569;">Credit Card Fee (${fees.creditCardFee}%)</td><td style="padding: 12px; text-align: right; font-size: 14px; color: #0f172a;">${moneyClient(calculations.ccFee)}</td></tr>`;
                                  ...
                                  <table className="tw-w-full tw-text-left tw-border-collapse">
                                      ...
                                      <tbody>
                                          ...
                                          {(() => {
                                              ...
                                              return (
                                                  <>
                                                    {Number(fees.otherFees || 0) > 0 && <tr><td colSpan="3" className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-600">Other Fees</td><td className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-900">{moneyClient(displayOtherFees)}</td></tr>}
                                                    {showFFILine && <tr><td colSpan="3" className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-600 tw-italic">FFI Fee (1.12%)</td><td className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-900">{moneyClient(calculations.ffiFee)}</td></tr>}
                                                    {creditCardFeeInclusion === 'separate' && Number(fees.creditCardFee || 0) > 0 && <tr><td colSpan="3" className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-600">Credit Card Fee ({fees.creditCardFee}%)</td><td className="tw-p-3 tw-text-right tw-text-sm tw-text-slate-900">{moneyClient(calculations.ccFee)}</td></tr>}
                                                  </>
                                              );
                                          })()}
                                      </tbody>
