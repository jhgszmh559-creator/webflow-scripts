  ...
  function InvoiceGenerator({ setupData, suppliers, onBack }) {
      ...
      const handleUpdateItem = (id, field, value) => {
          setItems(items.map(it => {
              ...
              // Automatically clear the selected supplier if the category changes
              if (field === 'category') {
                  updated.supplierId = "";
              }
                      ...
                      <ControlCard title={`Travel Services (${calculations.baseCurrency.code} Pricing)`} isComplete={isTravelComplete}>
                          <div className="tw-space-y-4">
                              {items.map((item) => {
                                  ...
                                  // NEW FILTERING LOGIC MATCHING YOUR SCHEMA
                                  const filteredSupplierOptions = suppliers
                                      .filter(s => {
                                          if (!s._supplier_type || !s._supplier_type.name) return true; // Keep uncategorized suppliers just in case
