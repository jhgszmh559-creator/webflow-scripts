(() => {
  ...
  function InvoiceGenerator({ setupData, suppliers, onBack, savedInvoiceData, currentQuoteId }) {
      ...
      const [documentType, setDocumentType] = useState(savedInvoiceData?.documentType || 'invoice');
      const [paymentCurrency, setPaymentCurrency] = useState(savedInvoiceData?.paymentCurrency || 'GBP');
      ...
      const calculatedDepositAmount = useMemo(() => {
          ...
          if (depositType === 'amount') {
              // Use the entered amount as-is. "100% upfront" items are assumed to be
              // already covered by the agent in the amount they typed.
              return depositValNum;
          }
                      ...
                      <ControlCard title={`Travel Services (${calculations.baseCurrency.code} Pricing)`} isComplete={isTravelComplete}>
                          <div className="tw-space-y-4">
                              ...
                              {(() => {
                                  ...
                                  const commissionBase = items.reduce((s, it) => s + (Number(it.nettUnitCost || 0) * Number(it.quantity || 0) * (Number(it.markup || 0) / 100)), 0);
                                  const grossBase = nettBase + commissionBase;
                      ...
                      <ControlCard title="Payments" defaultOpen={false} isComplete={isPaymentsComplete}>
                          <div className="tw-space-y-4">
                              <div>
                                  ...
                                  <div className="tw-grid tw-grid-cols-2 tw-gap-2 tw-p-1 tw-bg-slate-100 tw-rounded-lg">
                                      {['quote', 'invoice'].map(type => (
                                          <button
                                              key={type}
                                              type="button"
                                              onClick={() => setDocumentType(type)}
                                              className={`tw-h-9 tw-rounded-md tw-text-sm tw-font-semibold tw-capitalize tw-transition-colors ${documentType === type ? 'tw-bg-white tw-text-[#303350] tw-shadow-sm' : 'tw-bg-transparent tw-text-slate-500 hover:tw-text-slate-700'}`}
                                          >
                                              {type}
                                          </button>
                                      ))}
                                  </div>
                                  <p className="tw-text-xs tw-text-slate-500 tw-mt-1.5 tw-m-0">
                                      {documentType === 'quote'
                                          ? 'Quote: no bank details or payment link shown on the document.'
                                          : 'Invoice: includes bank account, payment currency and payment link.'}
                                  </p>
                              </div>
