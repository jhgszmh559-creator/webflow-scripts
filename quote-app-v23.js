(() => {
  ...
  const PRESET_EXCHANGE_RATES = {
      USD: 1.00, EUR: 0.95, GBP: 0.80, JPY: 150.00, AUD: 1.55, CAD: 1.35, CHF: 0.90,
  };
                  ...
                  <div style="max-width: 800px; margin: 0 auto;">
                      ...
                      ${paymentLink ? `
                          <div style="margin-top: 40px; text-align: right;">
                              <a href="${paymentLink}" target="_blank" style="display: inline-block; background-color: #0b0e2c; color: #ffffff; font-weight: bold; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-size: 16px;">Pay now</a>
                          </div>
                      ` : ''}
                  </div>
              </body>
                  ...
                  <div className="lg:tw-col-span-2 tw-space-y-6">
                      ...
                      <ControlCard title="Display Options" defaultOpen={false} isComplete={isDisplayComplete}>
                          <div className="tw-space-y-4">
                              ...
                              <div className="tw-flex tw-bg-slate-100 tw-p-1 tw-rounded-lg tw-w-full">
                                  {['summary', 'grouped', 'detailed'].map(v => (
                                      <button
                                          key={v}
                                          type="button"
                                          onClick={() => setInvoiceView(v)}
                                          className={`tw-flex-1 tw-text-sm tw-font-medium tw-py-2 tw-px-3 tw-rounded-md tw-transition-all tw-border-none tw-cursor-pointer ${invoiceView === v ? 'tw-bg-white tw-shadow-sm tw-text-[#303350]' : 'tw-bg-transparent tw-text-slate-500 hover:tw-text-slate-700'}`}
                                      >
                                          {v === 'grouped' ? 'By Category' : v.charAt(0).toUpperCase() + v.slice(1)}
                                      </button>
                                  ))}
                              </div>
                              {invoiceView === 'summary' && ( 
                          ...
                          <div className="tw-bg-white tw-rounded-2xl tw-shadow-[0_8px_30px_rgb(0,0,0,0.04)] tw-border tw-border-solid tw-border-slate-100 tw-p-8 sm:tw-p-12 tw-block">
                              ...
                              {paymentLink && (
                                  <div className="tw-mt-10 tw-text-right">
                                      <a href={paymentLink} target="_blank" rel="noopener noreferrer" className="tw-inline-block tw-bg-[#0b0e2c] tw-text-white tw-font-bold tw-py-3 tw-px-8 tw-rounded-lg tw-no-underline hover:tw-opacity-90 tw-transition-opacity tw-shadow-md">Pay now</a>
                                  </div>
                              )}
                          </div>
                      </div>
